...
def send_error_notification(error: str) -> None:
    """Send a notification about the given error to the relevant stakeholders."""
    # Implementation for sending error notifications

def retry_on_error(func: Callable, max_retries: int) -> Callable:
    """Retry the decorated function a maximum of `max_retries` times if it raises an error."""
