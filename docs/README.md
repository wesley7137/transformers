...
```bash
pip install -e ".[docs]" -r requirements.txt
```
...
```bash
doc-builder preview transformers docs/source/en/ --port 3000
```
...
The `preview` command only works with existing doc files. When you add a completely new file, you need to update `_toctree.yml` and restart the `preview` command (`ctrl-c` to stop it & call `doc-builder preview ...` again).
